...
void main() {
    ...
    float start=0.1;
    //float start=u_mouse.x/u_resolution.x;
    float w=0.1;
    //float w = (1.+sin(u_time))/2.; // rescaled to 0.0 .. 1.0;
    //float l = 1.0-(smoothstep(start-w, start, st.x) - smoothstep(start, start+w,st.x));

    //float l = step(start, st.x);
    //float l = step(start+w, st.x);
    //float l = step(start, st.x) - step(start+w, st.x);
    float b = 1.0;//step(start, st.y) - step(start+w, st.y);

    float l = rect(st, 0.1, 0.1, 0.25, 0.5);
